...
class TextAudioSpeakerCollate:
    ...
    def __call__(self, batch):
        ...
        ppg_lengths = ppg_lengths.numpy()
        pit_padded = pit_padded.numpy()
        ppg_padded = ppg_padded.numpy()
        spk = spk.numpy()
        spe_padded = spe_padded.numpy()
        ...
        # spe_lengths = np.asarray(spe_lengths,dtype=np.float32)
        # wav_padded = np.asarray(wav_padded,dtype=np.float32)
        # wav_lengths = np.asarray(wav_lengths,dtype=np.float32)
        return (
            ppg_padded,
            ppg_lengths,
            pit_padded,
